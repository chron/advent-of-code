...
def run(input)
  lines = input.split("\n")
  logs = lines.map { |l| l =~ /\[(.+)\] (.+)$/; [Time.parse($1), $2] }

  current_guard = nil
  sleep_time = nil
  sleeping_minutes = Hash.new(0)
  sleeping_at_minutes = Hash.new { |h,k| h[k] = Hash.new(0) }

  logs.sort.each do |time, message|
    if message =~ /Guard #(\d+)/
      current_guard = $1.to_i
    elsif message =~ /asleep/
      sleep_time = time
    else
      sleeping_minutes[current_guard] += ((time - sleep_time) / 60).floor

      sleep_time.min.upto(time.min) { |min| sleeping_at_minutes[current_guard][min] += 1 }
    end
  end

  guard, minutes = sleeping_minutes.to_a.max_by { |k,v| v }
  guard * sleeping_at_minutes[guard].max_by { |k,v| v }.first
end
...
test_cases.each do |input, output|
  raise "Test case failed:" if run(input) != output
end
